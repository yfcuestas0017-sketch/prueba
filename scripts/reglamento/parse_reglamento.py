...
# Paso 1 de 2 del pipeline de extraccion del reglamento. Ver README.md.
# La ruta al PDF se pasa por argumento: antes estaba codificada al escritorio
# de una de las autoras, lo que hacia el script irreproducible.

SCRIPT_DIR = Path(__file__).resolve().parent

if len(sys.argv) < 2:
    sys.exit(
        'Uso: python parse_reglamento.py <ruta-al-PDF>' + chr(10) +
        'PDF fuente: "ACUERDO 105 DE 2023 - Por el cual se expide el nuevo '
        'Reglamento de trabajo de grado y tesis.pdf"'
    )
...
with open(SCRIPT_DIR / 'reglamento_structured.json', 'w', encoding='utf-8') as f:
    json.dump(data, f, ensure_ascii=False, indent=2)

print('Generado:', SCRIPT_DIR / 'reglamento_structured.json')
